"""Project plugin config repository implementation for SQLite."""
...
class ProjectPluginConfigRepositorySQLite(ProjectPluginConfigRepository):
    """SQLite-specific implementation of ProjectPluginConfigRepository."""
    ...
    async def save(self, config: ProjectPluginConfig) -> ProjectPluginConfig:
        ...
        if existing:
            ...
            orm = ProjectPluginConfigMapper.to_orm(config, convert_uuid_to_string=True)
    ...
    async def find_by_id(self, config_id: UUID) -> Optional[ProjectPluginConfig]:
        ...
        orm = self._session.query(ProjectPluginConfigORM).filter(
            ProjectPluginConfigORM.id == query_id
        ).first()
    ...
    async def find_by_project_id(self, project_id: UUID) -> List[ProjectPluginConfig]:
        ...
        orms = self._session.query(ProjectPluginConfigORM).filter(
            ProjectPluginConfigORM.project_id == query_id
        ).all()
    ...
    async def find_by_project_and_plugin(
        ...
        project_query_id = str(project_id)
        plugin_query_id = str(plugin_id)
        orm = self._session.query(ProjectPluginConfigORM).filter(
            ProjectPluginConfigORM.project_id == project_query_id,
            ProjectPluginConfigORM.plugin_id == plugin_query_id,
        ).first()

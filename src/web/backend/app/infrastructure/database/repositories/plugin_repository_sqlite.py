"""Plugin repository implementation for SQLite."""
...
class PluginRepositorySQLite(PluginRepository):
    """SQLite-specific implementation of PluginRepository."""
    ...
    async def save(self, plugin: Plugin) -> Plugin:
        ...
        if existing:
            ...
            orm = PluginMapper.to_orm(plugin, convert_uuid_to_string=True)
    ...
    async def find_by_id(self, plugin_id: UUID) -> Optional[Plugin]:
        """Find plugin by ID."""
        query_id = str(plugin_id)
        orm = self._session.query(PluginORM).filter(
            PluginORM.id == query_id
        ).first()
